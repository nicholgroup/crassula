...
%                 Ex=Ex+Ex1+Ex2+q*Vf;
        ...
        Vh=diag(sum(qDensityMat.*invPosDiff./(4*pi*epsilon),2));
...
%                 Ex=Ex+q*Vf; on 2017/06/28
    ...
    VExEnergy=VExEnergy+real(psiF(:,j)'*Ex*psiF(:,j));%Exchange could give imaginary terms
end

energyElectrons=Ewoh+VHEnergy/2+VExEnergy/2;
